%Q5 白噪声的幅值改变对正弦信号的自相关函数的影响
clf;N=1000;Fs=1000;%数据长度和采样频率
n=0:N-1;t=n/Fs;%时间序列
Lag=100;%延迟样点数，限定返回值c的的长度2 x maxlags + 1
randn('state',0);%设置产生随机数的初始状态
x1=sin(2*pi*10*t)+1*randn(1,length(t));%x1原始信号
x2=sin(2*pi*10*t)+3*randn(1,length(t));%x2原始信号
