...
title('周期图 白噪声幅值为1');grid on;  %设置表头，保留图像

%第二种情况：N=1024
Fs=1000;
N=1024;Nfft=1024;  %数据长度和FFT所用的数据长度
n=0:N-1;t=n/Fs;  %采用的时间序列
% x2=sin(2*pi*100*t)+randn(1,N);  %不带有噪声的信号
x2=sin(2*pi*100*t)+3*randn(1,N);  %带有噪声的信号
...
xlabel('频率/Hz');ylabel('功率谱/dB');  %设置横纵坐标
title('周期图 白噪声幅值为3');grid on;  %设置表头，保留图像